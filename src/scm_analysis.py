...
Uso típico (CLI)
----------------
Procesar todos los ficheros .txt en un directorio::

    python -m src.scm_analysis --data-dir data/SPARC --out results/

Procesar mediante patrón glob::

    python -m src.scm_analysis --glob "data/SPARC/*.txt" --out results/
...
def _write_top10_latex(results: pd.DataFrame, out_dir: Path) -> None:
    ...
    try:
        ...
        lines = [
            ...
            r"\hline",
            r"galaxy & scm\_u\_term & chi2\_red & veredicto \\",
            r"\hline",
        ]
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    input_group.add_argument("--glob",         metavar="PATTERN", dest="glob_pattern",
                             help='Patrón glob, p. ej. "data/SPARC/*.txt".')
