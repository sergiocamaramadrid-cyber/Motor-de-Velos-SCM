...
def main(argv: list[str] | None = None) -> None:
    ...
    results = run_sensitivity(filepaths, delimiter=delimiter, out_dir=Path(args.out))
    logger.info("Análisis de sensibilidad completado: %d galaxias procesadas.", len(results))
    print(f"Sensibilidad calculada para {len(results)} galaxia(s).")
    for name, df in results.items():
        print(f"  {name}: {len(df)} puntos de barrido, "
              f"u_term rango [{df['scm_u_term'].min():.4f}, {df['scm_u_term'].max():.4f}]")
