...
def print_fit_summary(fit_result: dict, bins: list[dict], g_bar: np.ndarray) -> None:
    ...
    Cuando g0_hat toca el lower bound emite una línea [WARN] detallada que
    incluye log10(g0_hat) y lo compara con q10 y q50 de g_bar, de modo que
    se pueda distinguir si el límite inferior tiene sentido físico.
    ...
    print(
        f"[SCM v0.2] g0_hat={fit_result['g0_hat']:.4e}  "
    ...
    if fit_result["at_lower_bound"]:
        log10_g0 = fit_result["log10_g0_hat"]
        log10_q10 = np.log10(q["q10"])
        log10_q50 = np.log10(q["q50"])
        print(
            f"[WARN] g0_hat tocó el límite lower: "
            f"g0_hat={fit_result['g0_hat']:.4e}  "
            f"log10(g0_hat)={log10_g0:.3f}  "
            f"(q10 g_bar: log10={log10_q10:.3f}, "
            f"q50 g_bar: log10={log10_q50:.3f})"
        )
    if bins:
        first = bins[0]
        print(
