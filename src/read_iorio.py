...
Process a single file::
    ...
    python -m src.read_iorio path/to/NGC0024_rotmod.txt

Process all 17 galaxies listed in a text file (one path per line)::

    python -m src.read_iorio --batch galaxies.txt --out results/

Process a glob pattern::

    python -m src.read_iorio --glob "data/SPARC/Rotmod_LTG/*.txt" --out results/
"""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Read and validate SPARC/Iorio rotation-curve files.\n\n"
            "Column-index map used:\n"
            + "\n".join(f"  {idx}  {name}" for name, idx in MANDATORY_COLS.items())
            + "\n  6  sigma_V  (optional – velocity dispersion)"
        ),
    ...
    input_group.add_argument(
        ...
        dest="glob_pattern",
        metavar="PATTERN",
        help='Glob pattern for galaxy files, e.g. "data/SPARC/Rotmod_LTG/*.txt".',
    )

    parser.add_argument(
        "--delimiter",
        default="auto",
        help="Column delimiter: 'auto' (default), 'tab', ',', or any separator string.",
    )
    parser.add_argument(
        "--out",
        metavar="DIR",
        default=None,
        help="Directory to write per-galaxy CSV files (optional).",
    )
    parser.add_argument(
        "--summary",
        metavar="FILE",
        default=None,
        help="Write a batch summary CSV to FILE.",
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
...
def _resolve_filepaths(args: argparse.Namespace) -> list[Path]:
    """Return the list of galaxy file paths from the parsed CLI arguments."""
    if args.file:
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Processing %d galaxy file(s).", len(filepaths))
    ...
    for name, df in galaxies.items():
        ...
        if args.out:
            out_path = out_dir / f"{name}_parsed.csv"
    ...
    print(f"\nVeredicto SCM Framework: {n_ok}/{n_total} galaxias procesadas correctamente.")
    if n_ok < n_total:
        sys.exit(1)
