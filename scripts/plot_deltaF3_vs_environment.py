...
INPUT_CSV = RESULTS_DIR / "per_galaxy_delta_f3.csv"
OUTPUT_FIG = RESULTS_DIR / "fig_deltaF3_environment.png"
...
def main() -> int:
    ...
    required = {"logSigmaHI_out", "delta_f3"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Faltan columnas requeridas: {sorted(missing)}")

    df = df[["logSigmaHI_out", "delta_f3"]].dropna()
    if len(df) < 3:
    ...
    plt.ylabel("ΔF3 (outer rotation slope)")
    plt.title("Environmental Test: ΔF3 vs HI Surface Density")
