...
_EPS = 1e-300  # floor for log10; keeps log10(x) finite without distorting physical values
_MIN_SAMPLE = 8  # minimum finite-pair count for correlation / bootstrap tests
...
def _ensure_log10(x: pd.Series, assume_linear_if_gt0: bool = True) -> pd.Series:
    """
    If x looks linear positive, convert to log10. If already log-like (has negatives), keep.
    """
    xnum = _to_numeric(x)
    if not assume_linear_if_gt0:
        return xnum
    finite = xnum[np.isfinite(xnum)]
    if len(finite) == 0:
        return xnum
    # Heuristic: if most values are > 0 and range is small in log space? We'll keep simple:
    if np.nanmin(finite) > 0:
        # likely linear (e.g., g in m/s^2)
        return np.log10(np.maximum(xnum, _EPS))
    return xnum
...
def matched_pairs_delta(
    ...
    for i in idx:
        ...
        # candidates: similar mass, enough env separation
        cand = d[
        ...
        if len(cand) == 0:
            continue
        # sample a few to avoid dominance
        take = min(int(max_pairs_per_gal), len(cand))
        chosen = cand.sample(n=take, random_state=rng)
...
# Main
# -------------------------
def _fix_argv_negative(argv: List[str]) -> List[str]:
    """
    Allow --g0-bounds -16,-8 (space-separated negative value).
    Argparse mistakes '-16,-8' for a flag; join it with '=' when the next
    token after --g0-bounds starts with '-' and contains a comma.
    """
    import re
    out = []
    i = 0
    while i < len(argv):
        tok = argv[i]
        if tok == "--g0-bounds" and i + 1 < len(argv):
            nxt = argv[i + 1]
            if re.match(r"^-\d+(\.\d+)?,-\d+(\.\d+)?$", nxt):
                out.append(f"--g0-bounds={nxt}")
                i += 2
                continue
        out.append(tok)
        i += 1
...
def main():
    ...
    ap.add_argument("--g0-bounds", default="-16,-8", type=str, help="log10 bounds, e.g. '-16,-8' means (1e-16,1e-8).")
    ...
    args = ap.parse_args(_fix_argv_negative(sys.argv[1:]))
    ...
    # optional merge (env table, morphology, etc.)
    if args.env_merge:
        df2 = pd.read_csv(args.env_merge, encoding="utf-8-sig")
        # merge on autodetected galaxy col name (will use df's galaxy col)
        cm_tmp = autodetect_columns(df)
        galcol = cm_tmp.galaxy
        if galcol not in df2.columns:
            # try common alternatives in env table
    ...
    for gal, sub in df.groupby(cm.galaxy):
        ...
        # carry covariates at galaxy level if present
        for tag, col in [("mass", cm.mass), ("env", cm.env), ("morph", cm.morph), ("barred", cm.barred), ("gasfrac", cm.gasfrac)]:
            if col is not None and col in sub.columns:
                # take first non-null
                v = sub[col].dropna()
                row[col] = v.iloc[0] if not v.empty else np.nan
    ...
    # Dependency tests (if covariates exist)
    def corr_report(xcol: str, label: str):
        ...
        return {"covariate": label, "col": xcol, "spearman_rho": float(rho), "spearman_p": float(p), "kendall_tau": float(tau), "kendall_p": float(pk), "n": int(m.sum())}
    ...
    # Matched pairs (only if mass & env available)
    ...
    # Minimal "notarial" dump of what was detected
    detected = {
        "galaxy": cm.galaxy,
        "g_bar": cm.gbar,
        "g_obs": cm.gobs,
