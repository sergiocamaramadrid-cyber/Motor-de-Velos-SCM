...
rotation velocity) and tests whether the acceleration-anomaly residual
(``a_residual`` = median log10(g_obs / g_bar)) differs between those groups
using the Mann-Whitney U test.

Naming note
-----------
``a_residual`` is the **acceleration-anomaly observable** (A_residual): the
per-galaxy median of log10(g_obs / g_bar).  It is intentionally distinct from
``F3_geom`` (the geometric deep-regime slope β = friction_slope, produced by
``generate_f3_catalog.py``).  Conflating the two would mix a scalar offset
with a slope, which are different physical quantities.

The test addresses the question: *Does the acceleration anomaly depend on the
asymptotic rotation velocity of the galaxy?*  A significant result (small p)
indicates that ``a_residual`` is correlated with ``v_last``, which may point
...
``results/scm_clean_with_residual.csv``  (or ``--input PATH``)
  Required columns: ``a_residual``, ``v_last``

Output
------
Printed summary:
  - Sample sizes for low- and high-``v_last`` groups
  - Median ``a_residual`` in each group
...
REQUIRED_COLS = ["a_residual", "v_last"]
...
def load_residual_catalog(csv_path: Path) -> pd.DataFrame:
    ...
    -------
    pd.DataFrame
        Validated catalog with at least ``a_residual`` and ``v_last`` columns.
...
def analyze_residual_by_v_last(df: pd.DataFrame) -> dict:
    """Run the Mann-Whitney U test comparing acceleration-anomaly residuals by v_last group.

    Rows with non-finite ``a_residual`` or ``v_last`` are dropped before
    analysis.

    Parameters
    ----------
    df : pd.DataFrame
        Catalog with columns ``a_residual`` and ``v_last``.
    ...
    dict
        ...
        - ``median_low`` / ``median_high``: group medians of ``a_residual``
        - ``v_last_median``: the split threshold
        - ``statistic``: Mann-Whitney U statistic
        - ``p_value``: two-sided p-value
    """
    df_clean = df[
        np.isfinite(df["a_residual"]) & np.isfinite(df["v_last"])
    ].copy()
    ...
    low = df_clean.loc[df_clean["v_last"] <= median_v, "a_residual"]
    high = df_clean.loc[df_clean["v_last"] > median_v, "a_residual"]
...
def print_results(results: dict) -> None:
    ...
    sep = "=" * 60
    print(sep)
    print("  Motor de Velos SCM — A_residual vs V_last Analysis")
    print(sep)
    print(f"  Total galaxies (clean): {results['n_total']}")
    print(f"  Split threshold v_last median: {results['v_last_median']:.2f} km/s")
    print()
    print("  Low vs High V_last — A_residual (acceleration anomaly) comparison")
    print(f"    n_low  = {results['n_low']},  median a_residual = {results['median_low']:.4f}")
    print(f"    n_high = {results['n_high']},  median a_residual = {results['median_high']:.4f}")
    print()
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Mann-Whitney U test: does a_residual (acceleration anomaly) differ between "
