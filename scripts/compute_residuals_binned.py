...
After fitting the script prints:
    ...
    [WARN] g0_hat tocó el límite lower: g0_hat=<value>  log10(g0_hat)=<value>  q10(g_bar)=<value>  q50(g_bar)=<value>
    [WARN] g0_hat << q10(g_bar): posible problema físico o de unidades
    [WARN] g0_hat hit UPPER bound

The WARN lines are only printed when the fitted log10(g0) is within
G0_BORDER_TOL decades of either bound of the search interval.  The second
lower-bound WARN (g0_hat << q10) is printed only when g0_hat falls below
the 10th percentile of the baryonic acceleration distribution.
...
def print_telemetry(
    ...
    When g0_hat touches the lower bound, also emits:
      [WARN] g0_hat tocó el límite lower: g0_hat=...  log10(g0_hat)=...  q10(g_bar)=...  q50(g_bar)=...
      [WARN] g0_hat << q10(g_bar): posible problema físico o de unidades
        (the second line is printed only when g0_hat < q10(g_bar))

    When g0_hat touches the upper bound, also emits:
      [WARN] g0_hat hit UPPER bound
    ...
    if fit["touches_lower"]:
        log10_g0 = np.log10(fit["g0_hat"])
        print(
            f"[WARN] g0_hat tocó el límite lower: g0_hat={fit['g0_hat']:.4e}"
            f"  log10(g0_hat)={log10_g0:.2f}"
            f"  q10(g_bar)={q10:.4e}  q50(g_bar)={q50:.4e}"
        )
        if fit["g0_hat"] < q10:
            print(
                "[WARN] g0_hat << q10(g_bar): posible problema físico o de unidades"
