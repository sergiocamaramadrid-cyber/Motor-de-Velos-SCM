...
class TestComputeF3FromRc:
    ...
    def test_fallback_when_ext_region_empty(self):
        """Single-point curve: ext region empty, should fall back to last K."""
        # 3 points all equal to r_max → ext_frac * r_max == r_max,
        # so none satisfy r > ext_frac * rmax → fallback fires
        df = pd.DataFrame({
            "r_kpc": [1.0, 1.0, 1.0],
            "Vbary_kms": [30.0, 30.0, 30.0],
        })
        params = HingeParams(ext_frac=1.0)  # forces empty ext region
