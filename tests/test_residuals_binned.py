...
class TestPrintTelemetry:
    def _make_fit(self, g0_hat=1.2e-10, touches_lower=False, touches_upper=False):
        return {
            "g0_hat": g0_hat,
            "g0_lo": g0_hat * 0.8,
            "g0_hi": g0_hat * 1.2,
    ...
    def test_warn_lower_includes_g0_hat_value(self, capsys):
        """Lower-bound WARN must include g0_hat=... with the fitted value."""
        g_bar = np.logspace(-13, -10, 30)
        fit = self._make_fit(g0_hat=1.2e-10, touches_lower=True)
        print_telemetry(fit, g_bar)
        captured = capsys.readouterr()
        assert "[WARN] g0_hat tocó el límite lower: g0_hat=" in captured.out

    def test_warn_lower_includes_log10_g0_hat(self, capsys):
        """Lower-bound WARN must include log10(g0_hat)=... for diagnostic use."""
        g_bar = np.logspace(-13, -10, 30)
        fit = self._make_fit(g0_hat=1.2e-10, touches_lower=True)
        print_telemetry(fit, g_bar)
        captured = capsys.readouterr()
        assert "log10(g0_hat)=" in captured.out

    def test_warn_lower_includes_q10_q50_g_bar(self, capsys):
        """Lower-bound WARN must include q10(g_bar) and q50(g_bar) for comparison."""
        g_bar = np.logspace(-13, -10, 30)
        fit = self._make_fit(g0_hat=1.2e-10, touches_lower=True)
        print_telemetry(fit, g_bar)
        captured = capsys.readouterr()
        assert "q10(g_bar)=" in captured.out
        assert "q50(g_bar)=" in captured.out

    def test_warn_lower_physical_note_when_g0_below_q10(self, capsys):
        """When g0_hat < q10(g_bar), a physical-issue note must be printed."""
        # g_bar range: [1e-13, 1e-10]; q10 ≈ 2e-13
        g_bar = np.logspace(-13, -10, 30)
        q10 = np.quantile(g_bar, 0.10)
        # Force g0_hat well below q10
        g0_small = q10 * 0.001
        fit = self._make_fit(g0_hat=g0_small, touches_lower=True)
        print_telemetry(fit, g_bar)
        captured = capsys.readouterr()
        assert "g0_hat << q10(g_bar)" in captured.out

    def test_warn_lower_no_physical_note_when_g0_above_q10(self, capsys):
        """When g0_hat >= q10(g_bar), the physical-issue note must NOT be printed."""
        g_bar = np.logspace(-13, -10, 30)
        q10 = np.quantile(g_bar, 0.10)
        # Force g0_hat above q10
        g0_above = q10 * 10.0
        fit = self._make_fit(g0_hat=g0_above, touches_lower=True)
        print_telemetry(fit, g_bar)
        captured = capsys.readouterr()
        assert "g0_hat << q10(g_bar)" not in captured.out
